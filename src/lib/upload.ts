...
export async function uploadSong(
  ...
  const workerResp = await fetch(`${WORKER_URL.replace(/\/+$/, '')}/upload`, {
    method: 'POST',
    headers: {
  ...
  if (!workerResp.ok) {
    ...
    throw new Error(`worker /upload ${workerResp.status}: ${text}`);
  }
...
export async function uploadFromYoutube(
  ...
  const workerResp = await fetch(`${WORKER_URL.replace(/\/+$/, '')}/upload_youtube`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      Authorization: `Bearer ${accessToken}`,
  ...
  if (!workerResp.ok) {
    const text = await workerResp.text();
    throw new Error(`worker /upload_youtube ${workerResp.status}: ${text}`);
  }
  const { job_id: jobId } = (await workerResp.json()) as { job_id: string };
